...
// Fixed version of parseMarkdownTree with EmphasisMark in skip list
function parseMarkdownTreeFixed(node: SyntaxNode, input: string): MDNode {
  ...
  if (child && textContainerTypes.includes(node.name)) {
    ...
    while (child) {
      ...
      const skipInParsing = [
        ...
        'TableDelimiterRow',
        'LinkMark',
        'EmphasisMark', // FIXED: Added EmphasisMark to skip list
      ]
      ...
      children.push(parseMarkdownTreeFixed(child, input))
      lastEnd = child.to
      child = child.nextSibling
    }
...
testCases.forEach(testInput => {
  ...
  console.log('\nFixed parsing result:')
  console.log('-'.repeat(60))
  
  const tree = parser.parse(testInput)
  const mdNode = parseMarkdownTreeFixed(tree.topNode, testInput)
  printMDNode(mdNode)
})