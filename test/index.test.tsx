...
describe('MdastRenderer', () => {
    ...
    />)
    expect(asFragment()).toRenderEqual("<p>Use <code>console.log()</code> to debug.</p>")
  })
    ...
    />)
    expect(asFragment()).toRenderEqual("<pre><code>const x = 1;\nconsole.log(x);</code></pre>")
  })
    ...
    />)
    expect(asFragment()).toRenderEqual("<pre><code>const x = 1;</code></pre><p>console.log(x);</p>")
  })
    ...
    const { asFragment } = render(() => <MdastRenderer 
      content={"> This is a **bold** statement with [a link](https://example.com)\n> \n> And this is *italic* text in the same blockquote" }
    ...
    const { asFragment } = render(() => <MdastRenderer 
      content={"> This blockquote contains a list:\n> \n> 1. First item\n> 2. Second item\n> 3. Third item" }
    ...
    const { asFragment } = render(() => <MdastRenderer 
      content={"1. First item with multiple paragraphs.\n\n   This is the second paragraph of the first item.\n\n2. Second item with code:\n\n   ```javascript\n   console.log('Hello from list');\n   ```" }
      
      
    />)
    expect(asFragment()).toRenderEqual("<ol ><li><p>First item with multiple paragraphs.</p><p>This is the second paragraph of the first item.</p></li><li><p>Second item with code:</p><pre><code>console.log('Hello from list');</code></pre></li></ol>")
  })
    ...
    const { asFragment } = render(() => <MdastRenderer 
      content={"[**Bold link**](https://example.com) and [*italic link*](https://test.com)" }
    ...
    const { asFragment } = render(() => <MdastRenderer 
      content={"[Check out `console.log()`](https://developer.mozilla.org)" }
      
      
    />)
    expect(asFragment()).toRenderEqual("<p><a href=\"https://developer.mozilla.org\" target=\"_blank\" rel=\"noopener noreferrer\">Check out <code>console.log()</code></a></p>")
  })
      it('renders inline code with emphasis markers that should not be processed', () => {
    const { asFragment } = render(() => <MdastRenderer 
      content={"`const **bold** = 'not bold';`" }
      
      
    />)
    expect(asFragment()).toRenderEqual("<p><code>const **bold** = 'not bold';</code></p>")
  })
    ...
    />)
    expect(asFragment()).toRenderEqual("<ol ><li>Install dependencies:<pre><code>npm install</code></pre></li><li>Run the application:<pre><code>npm start</code></pre></li></ol>")
  })
    ...
    />)
    expect(asFragment()).toRenderEqual("<ol ><li><p>Install dependencies:</p><pre><code>npm install</code></pre><ol ><li>Run the application:<pre><code>npm start</code></pre></li></ol></li></ol>")
  })
    ...
    />)
    expect(asFragment()).toRenderEqual("<blockquote><p>Here's an example:</p><pre><code>function hello() {\n  return \"world\";\n}</code></pre></blockquote>")
  })
    ...
    const { asFragment } = render(() => <MdastRenderer 
      content={"This has **bold** and **more bold** and *italic* and *more italic* text." }
    ...
    const { asFragment } = render(() => <MdastRenderer 
      content={"This is **bold text that\ncontinues on the next line**" }
    ...
    const { asFragment } = render(() => <MdastRenderer 
      content={"***This is bold and italic*** with ~~strikethrough **and bold**~~" }
      extensions={[extensions["gfm"]()]}
      mdastExtensions={[mdastExtensions["gfmFromMarkdown"]()]}
    />)
